...
class VideoTracker(object):
    ...
    def __enter__(self):
        ...
        if self.args.save_path:
            ...
            self.save_video_path = os.path.join(self.args.save_path, f"{now_time}.mp4") if self.args.save_video else os.path.join(self.args.save_path, "resrults.mp4")
    ...
    def run(self):
        ...
        idx_frame = 0
        last_out = None
        get_image = rec.start_server(False) if self.args.get_from_http else lambda: self.vdo.retrieve()[1]
        try:
            while self.vdo.grab():
                # Inference *********************************************************************
                t0 = time.time()
                # self.vdo: カメラor動画のcv2.VideoCaptureが保持
                # ここをHTTP画像受信に変更したら、ストリーミング検出対応できるかも

                img0 = get_image()
                # _, img0 = self.vdo.retrieve()
                # img0 = rec.start_server(False)    # うまく行った方
                ...
                if len(outputs) > 0:
                    ...
                    # 物体のラベルと総数を表示
                    unique_ids = np.unique(identities)  # 一意のIDを取得
                    total_count = len(unique_ids)  # 総数を計算
                    print(f'Detected objects: {unique_ids}, Total count: {total_count}')  # ラベルと総数を表示


                    #*************************************************
                    # JSON形式で人数を保存
                    now_time = datetime.datetime.now().timestamp()
                    number_of_people[f"{now_time}"] = total_count
        ...
        # カメラ駆動の時にCtrl+Cで正常終了するようにする。
        except KeyboardInterrupt:
            print("\nKeyboard Interrupt detected. Exiting the loop.")


        # 検出人数をJSON記録
        json_file = "./record.json"
        with open(json_file, 'a') as file:  # 'a' モードで追記
            json.dump(number_of_people, file)
            file.write('\n')  # 各エントリを新しい行に分ける
...
if __name__ == '__main__':
    ...
    parser.add_argument("--get_from_http", action="store_true")
